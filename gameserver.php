...
//print_r($this_server);
$sql = "SELECT player_history.`steam_id`, players.name_c, player_history.`game_time`, players.country, player_history.`last_play` FROM `player_history` INNER JOIN players ON player_history.steam_id = players.steam_id64 WHERE `game` LIKE '$bserver' ORDER BY `game_time` DESC;";
