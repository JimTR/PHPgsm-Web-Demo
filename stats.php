...
foreach($dups as $dup) {
	...
	if ($dup['ip'] == $last_ip) {
		...
		$dup_table[$i]['name'] .="<table><tr class='no-border'><td style='width:50%'><a href='javascript:void(0)' class='user-id' id='$id' title='Last Seen $last_login'>{$dup['name_c']}</a></td><td style='width:45%;'>$last_login</td><td style='text-align:right;padding-right:3%;'>{$dup['log_ons']}</td></tr></table>";
		$i++;
		continue;
	}
