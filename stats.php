...
$sql = "SELECT *,count('id') as total, sum(game_time) as total_time FROM `user_stats` order by `time_on_line` desc limit 1;";
$stats = $database->get_results($sql);
$page['player_total'] = $stats[0]['total'];
...
$sql = 'SELECT * FROM `user_stats` order by `total_log_ons` desc limit 1;';
$stats = $database->get_results($sql);
$page['most_log_ons'] = $stats[0]['name'];
$page['log_on_count'] = $stats[0]['total_log_ons'];
$sql ="SELECT player_history.game, servers.server_name, sum(player_history.log_ons) as total FROM `player_history` left join `servers` on `servers`.`host_name` = player_history.game group by game order by total DESC limit 1;";
$stats = $database->get_results($sql);
